...
\texttt{agent/graph.py} & LangGraph \texttt{StateGraph} definition, async checkpointer, cached tool node \\
...
\subsection{LangGraph Node/Edge Visualization}

\begin{figure}[h!]
\centering
\begin{tikzpicture}[
  node distance=2.8cm,
  box/.style={draw=fridaybrand, rounded corners, thick, minimum width=3.1cm, minimum height=1.0cm, align=center, fill=blue!4},
  edge/.style={-{Latex[length=2.8mm]}, thick, draw=fridaybrand}
]
\node[box] (start) {START};
\node[box, right=of start] (agent) {agent node\\(LLM + tools)};
\node[box, right=of agent] (tools) {tools node\\(Tool executor)};
\node[box, below=of agent] (endn) {END};

\draw[edge] (start) -- (agent);
\draw[edge] (agent) -- node[above, font=\small] {tool\_calls} (tools);
\draw[edge] (tools) -- node[above, font=\small] {tool results} (agent);
\draw[edge] (agent) -- node[right, font=\small] {no tool\_calls} (endn);
\end{tikzpicture}
\caption{Friday's LangGraph control flow (nodes and directed edges).}
\end{figure}

\begin{notebox}
The graph is intentionally cyclic (agent $\leftrightarrow$ tools).  This is what enables iterative ReAct behavior: the LLM can inspect tool output and decide the next action until completion.
\end{notebox}

% ══════════════════════════════════════════════════════════════
\section{Architecture Decisions (Why)}

\begin{table}[h!]
\centering
\begin{tabular}{@{}p{3.4cm}p{5.4cm}p{5.0cm}@{}}
\toprule
\textbf{Decision} & \textbf{Why This Choice} & \textbf{Trade-off Accepted} \\
\midrule
LangGraph ReAct loop & Needed explicit cyclic control flow for tool use and retries instead of one-shot prompting. & More state management complexity than a linear chain. \\
FastAPI + SSE & Streaming intermediate thought/tool events improves observability and UX responsiveness. & SSE parsing/dedup logic is more complex than plain JSON responses. \\
Sandbox + allowlist shell policy & Reduces blast radius for command execution and path traversal risks. & Some legitimate commands require explicit allowlist updates. \\
Skill library + dynamic tool loading & Makes capabilities persistent and reusable across sessions; enables self-improvement. & Requires index management and tool-name collision handling. \\
Skill Agents (framework manifests) & Encodes framework-specific best practices so outputs stay consistent over time. & Adds maintenance overhead for guide quality/versioning. \\
Dynamic MCP registry tools & Lets Friday learn external API-style tools at runtime instead of hardcoding all integrations. & Runtime registration can fail if endpoint metadata is poor. \\
Prompt/context budgeting & Prevents local-model failures on small context windows (e.g., 4k) by trimming prompt/history. & Some long-tail context can be dropped on very long chats. \\
Local-first LM Studio default & Fast local iteration without cloud spend/rate limits. & Smaller local models may need stricter prompt discipline. \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Tool} & \textbf{Description} \\
\midrule
\texttt{get\_stock\_quote} & Fetches quote data for a ticker using Yahoo Finance (price, change, market cap, P/E where available) \\
\texttt{compare\_stock\_prices} & Fetches and compares two ticker symbols in a single call \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Tool} & \textbf{Description} \\
\midrule
\texttt{register\_mcp\_tool} & Registers an external endpoint at runtime; creates dynamic tool \texttt{mcp\_<name>} \\
\texttt{list\_registered\_mcp\_tools} & Lists currently registered MCP tools from \texttt{skills/mcp\_tools.json} \\
\texttt{unregister\_mcp\_tool} & Removes a runtime-registered MCP tool \\
\texttt{mcp\_call} & Directly invokes a registered MCP tool by server/name \\
\bottomrule
\end{tabular}
\end{table}
  ...
  friday-backend/
    ...
    agent/
      ...
      tools/
        ...
        market_data.py        # Yahoo Finance quote/compare tools
        mcp_registry.py       # Runtime MCP registration + dynamic tools
    ...
    skills/
      index.json              # Script skill registry
      mcp_tools.json          # Runtime MCP tool registry
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Richer MCP transport:} Add full protocol-level MCP transport/session support beyond simple HTTP endpoint wrappers.
...
\begin{notebox}
\textbf{Recent Changes (v0.2.0):}
\begin{itemize}[leftmargin=*]
  \item Migrated from synchronous \texttt{SqliteSaver} to \texttt{AsyncSqliteSaver} for proper async checkpoint persistence.
  \item Replaced \texttt{threading.Lock} with \texttt{asyncio.Lock} in the dynamic tool node cache for async-safe synchronization.
  \item Graph initialization is now lazy via \texttt{get\_graph()} to support async checkpointer setup.
  \item Added \texttt{aiosqlite} dependency for async SQLite operations.
\end{itemize}
\end{notebox}

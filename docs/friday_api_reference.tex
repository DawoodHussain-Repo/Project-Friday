...
\subsubsection{get\_stock\_quote}

\begin{tabular}{@{}lp{10cm}@{}}
\textbf{Arg} & \texttt{symbol: str} --- ticker symbol (e.g., NVDA) \\
\textbf{Provider} & Yahoo Finance via \texttt{yfinance} \\
\textbf{Returns} & JSON with current price, daily change, market cap, P/E (when available) \\
\end{tabular}

\subsubsection{compare\_stock\_prices}

\begin{tabular}{@{}lp{10cm}@{}}
\textbf{Args} & \texttt{symbol\_a: str}, \texttt{symbol\_b: str} \\
\textbf{Returns} & JSON with both quotes plus computed deltas/ratio \\
\end{tabular}
...
\subsubsection{register\_mcp\_tool}

\begin{tabular}{@{}lp{10cm}@{}}
\textbf{Args} & \texttt{tool\_name}, \texttt{description}, \texttt{endpoint}, \texttt{method=POST}, \texttt{server\_name=default}, \texttt{headers\_json=\"\{\}\"} \\
\textbf{Action} & Saves endpoint metadata to \texttt{skills/mcp\_tools.json} and exposes dynamic tool \texttt{mcp\_<tool\_name>} \\
\textbf{Returns} & Registration confirmation or validation error \\
\end{tabular}

\subsubsection{list\_registered\_mcp\_tools}

\begin{tabular}{@{}lp{10cm}@{}}
\textbf{Args} & None \\
\textbf{Returns} & JSON list of registered MCP tool definitions \\
\end{tabular}

\subsubsection{unregister\_mcp\_tool}

\begin{tabular}{@{}lp{10cm}@{}}
\textbf{Arg} & \texttt{tool\_name: str} \\
\textbf{Returns} & Confirmation or not-found message \\
\end{tabular}

\subsubsection{mcp\_call}

\begin{tabular}{@{}lp{10cm}@{}}
\textbf{Args} & \texttt{server\_name: str}, \texttt{tool\_name: str}, \texttt{payload: str=\"\{\}\"} \\
\textbf{Action} & Executes a registered MCP entry by server/name \\
\textbf{Returns} & Endpoint response text/JSON or error string \\
\end{tabular}
...
\texttt{MAX\_HISTORY\_MESSAGES} & \texttt{12} & Max conversation messages sent to LLM each turn \\
\texttt{MAX\_SKILL\_CONTEXT\_CHARS} & \texttt{1800} & Max injected skill-context length in system prompt \\
...
\texttt{COMMAND\_DENYLIST} & \texttt{curl,wget} & Explicitly blocked command prefixes (wins over allowlist) \\
\texttt{MCP\_TIMEOUT\_SECONDS} & \texttt{20} & Timeout for dynamic MCP endpoint calls \\
